...
#now includes reward 
#now includes new terminal states for walls and shelves.
import numpy as np
...
warehouse, size = return_warehouse('D') 
...
#experimental
number_of_visits = np.zeros((size, size))
#initialize networks:
...
ep = 0
counter = 0
while len(valid_starting_squares) > 0:
    ep += 1
    state_i, state_j = random.choice(valid_starting_squares)
    if number_of_visits[state_i, state_j] >= 50:
        valid_starting_squares.remove((state_i, state_j))
    if len(valid_starting_squares) == 0 and ep < num_episodes:
        number_of_visits = np.zeros((size, size))
        counter += 1 #using epochs
        valid_starting_squares = []
        for i in range(size):
            for j in range(size):
                if warehouse[i,j] == 0:
    ...
    for step in range(num_steps):
        number_of_visits[state_i, state_j] += 1
        if not terminated:
            ...
            if warehouse[new_state_i, new_state_j] in [TARGET, SHELF, WALL]: #additional terminal states for shelves and walls.
                terminated = True

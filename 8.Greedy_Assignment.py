...
def assignment(target_list, robot_current_locations):
    n_r = len(robot_current_locations)
    n_t = len(target_list)
    ...
    values = np.array([
        [target_list[t].V_table[robot_current_locations[r]] for t in range(n_t)]
        for r in range(n_r)
    ])
...
for i in range(N_ROBOTS):
    ...
    sum += average_steps[i]
mean_path_lengths_ps=(sum/N_ROBOTS)
...
run_matplotlib = input("Run matplotlib Y/N: ")
if run_matplotlib == 'Y':
    fig, ax = plt.subplots(figsize=(10, 10))
    ax.imshow(warehouse[:, :], cmap=cmap)
...
#pygame simulation
run_pygame_simulation = input("Run Pygame simulation Y/N: ")
while run_pygame_simulation == 'Y':
    WINDOW_SIZE = 600
    ...
    pygame.quit()
    run_pygame_simulation = input("Run Pygame simulation again? Y/N: ")
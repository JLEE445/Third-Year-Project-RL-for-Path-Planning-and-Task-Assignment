...
plt.savefig('4.Rewards_Per_Episode.png')

#best_actions:
num_agents = 2
all_best_q_vals = np.zeros((size, size, num_agents))
...
for i in range(size):
    for j in range(size):
        ...
        with torch.no_grad():
            ...
            all_best_q_vals[i,j,0] = qvals.max().item()
...
for i in range(size):
    for j in range(size):
        ...
        with torch.no_grad():
            ...
            all_best_q_vals[i,j,1] = qvals.max().item()
combined_best_actions = np.zeros((size, size), dtype=int)
for i in range(size):
    for j in range(size):
        if best_actions_A[i,j] == best_actions_B[i,j]:
            combined_best_actions[i,j] = best_actions_A[i,j]
        else:
            agent = all_best_q_vals[i,j,:].argmax()
            if agent == 0:
                combined_best_actions[i,j] = best_actions_A[i,j]
            else:
                combined_best_actions[i,j] = best_actions_B[i,j]
print(all_best_q_vals[4,8,:])
print(all_best_q_vals[4,1,:])
fig, ax = plt.subplots(figsize=(10,10))
ax.imshow(warehouse[:,:], cmap = cmap)
...
for i in range(size):
    for j in range(size):
        dx = dy = 0
        if combined_best_actions[i,j] == 0: #up
            dy = -0.3
        elif combined_best_actions[i,j] == 1:#right
            dx = 0.3
        elif combined_best_actions[i,j] == 2:#down
            dy = 0.3
        elif combined_best_actions[i,j] == 3:#left
            dx = -0.3
        ax.arrow(j, i, dx, dy, color='black', head_width=0.2, head_length=0.1)
plt.savefig('4.Combined_Learned_Policy.png')
path_i_A = []
path_j_A = []
